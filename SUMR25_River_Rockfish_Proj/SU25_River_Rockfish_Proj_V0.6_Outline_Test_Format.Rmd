...
**Balanced Design**
        ...
        - Tukey HSD test: Parametric test
...
Non-Parametric Two-Way ANOVA Like Tests: Options include
- Scheirer-Ray-Hare test (non-parametric)

- Aligned Ranked transformation (ART) ANOVA: Nonparametric test, robust to non-normally distributed data and data with unequal variance. Designed to work with two or more factors and their effects.
...
For datasets testing for interactions across multiple factors with unequal sample sizes between levels, type II and type III sum of squares is often recommended opposed to type I. 

Note: If interaction is significant, a two-way ANOVA test should not be utilized??? (Klasson Thomas, 2020)

Type II sum of squares: May hold greater power than type III, so use for unbalanced dataset if low interaction is being detected. Type II test is sensitive to how unbalanced the data is, and use with caution when assessing interactive effects.

Type III sum of squares: Also recommended for unbalanced datasets. 
       > library(car) 
       > pH_aov_int <- aov(pH ~ Pregnant.Or.Atresia * Ambient.Or.OAH, data = general_samples)
     ...
     - Unbalanced nonparametric Scheirer Ray Hare test:
       ...
       > scheirerRayHare(pH ~ Pregnant.Or.Atresia + Ambient.Or.OAH, data = general_samples, type = 2)
       
     - Uses a type-I sum-of-squares and will display different values than the type II-sum-of-squares when unbalanced.

     - For unbalanced data consider type II ot type III sum of squares design
